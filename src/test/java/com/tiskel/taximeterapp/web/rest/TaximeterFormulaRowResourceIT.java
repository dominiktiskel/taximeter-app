...
class TaximeterFormulaRowResourceIT {
    ...
    public static TaximeterFormulaRow createEntity(EntityManager em) {
        ...
        // Add required entity
        TaximeterFormula taximeterFormula;
        if (TestUtil.findAll(em, TaximeterFormula.class).isEmpty()) {
            taximeterFormula = TaximeterFormulaResourceIT.createEntity(em);
            em.persist(taximeterFormula);
            em.flush();
        } else {
            taximeterFormula = TestUtil.findAll(em, TaximeterFormula.class).get(0);
        }
        taximeterFormulaRow.setFormula(taximeterFormula);
    ...
    public static TaximeterFormulaRow createUpdatedEntity(EntityManager em) {
        ...
        TaximeterFormula taximeterFormula;
        if (TestUtil.findAll(em, TaximeterFormula.class).isEmpty()) {
            taximeterFormula = TaximeterFormulaResourceIT.createUpdatedEntity(em);
            em.persist(taximeterFormula);
            em.flush();
        } else {
            taximeterFormula = TestUtil.findAll(em, TaximeterFormula.class).get(0);
    ...
    void createTaximeterFormulaRow() throws Exception {
        ...
        TaximeterFormulaRowDTO taximeterFormulaRowDTO = taximeterFormulaRowMapper.toDto(taximeterFormulaRow);
        restTaximeterFormulaRowMockMvc
            .perform(
                post(ENTITY_API_URL)
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(taximeterFormulaRowDTO))
            )
    ...
    void putNewTaximeterFormulaRow() throws Exception {
        ...
        TaximeterFormulaRowDTO taximeterFormulaRowDTO = taximeterFormulaRowMapper.toDto(updatedTaximeterFormulaRow);

        restTaximeterFormulaRowMockMvc
            .perform(
                put(ENTITY_API_URL_ID, taximeterFormulaRowDTO.getId())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(taximeterFormulaRowDTO))
            )
            .andExpect(status().isOk());
    ...
    void patchNonExistingTaximeterFormulaRow() throws Exception {
        ...
        restTaximeterFormulaRowMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, taximeterFormulaRowDTO.getId())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(taximeterFormulaRowDTO))
            )
            .andExpect(status().isBadRequest());
